...
###5.3.2 FIG.6B-D / Mean +/- Sem bar graphs condition (Figure 6B-D)
...
###5.3.3 FIG.3A / Sankey diagramm for visualizing distribution of DEGs in biological contrasts (Figure 3A)
...
###5.3.4 FIG.3B / DEGs Barplot per cell type (Figure 3B)
```{r}
outputFolder <- "/media/Helios_scStorage/Mariano/Human_Mice_Comparison/Human_Mice_Integrated/Contrast_Cell_specific/"
...
###5.4.6 FIG.5A / GO (Database) GSEA results in Heatmap using simplifyEnrichment for HFrEF (Figure 5A)
```{r}
outputFolder <- "/media/Helios_scStorage/Mariano/Human_Mice_Comparison/Human_Mice_Integrated/Contrast_Cell_specific/GSEA_analysis"
...
###5.4.7 FIG.6A / building Dot plot with GSEA result Cardiomyocytes (Figure 6A)
```{r}
...
#### 5.4.7.1 SUPP.FIG.5A-D / GO (Database) GSEA results in Dot plot for HFrEF Endothelial (Supplement Figure 5A-D)
```{r}
outputFolder <- "/media/Helios_scStorage/Mariano/Human_Mice_Comparison/Human_Mice_Integrated/Contrast_Cell_specific/GSEA_analysis"
...
#5.4.8 SUPP.FIG2 SeuratObject UMAPs for all analyzed tools (Supplement Figure 2)
```{r}
outputFolder <-  "/media/Helios_scStorage/Mariano/Revision_scib/Supplement_Figure_2/"

#Read in Object to use
Seu.Obj <- readRDS("/media/Storage/R/Integrated/SeuratObject.combined.integrated.annotated_10_08_22.rds")
DimPlot(Seu.Obj, group.by = "seurat_clusters", label = T)
ggsave(filename = paste(outputFolder,"Dimplot_OrthoIntegrate_10_08_22_seuratclusters.png"), device = "png", width = 10, height = 8)
DimPlot(Seu.Obj, group.by = "cell_type", label = T)
ggsave(filename = paste(outputFolder,"Dimplot_OrthoIntegrate_10_08_22_celltype.png"), device = "png", width = 10, height = 8)

#Repeat for other Objects Biomart
Seu.Obj.biomart <- readRDS("/media/Storage/R/just_biomart/SeuratObject.combined.integrated_10_08_22_annotated.rds")
DimPlot(Seu.Obj.biomart, group.by = "seurat_clusters", label = T)
ggsave(filename = paste(outputFolder,"Dimplot_biomart_10_08_22_seuratclusters.png"), device = "png", width = 10, height = 8)
DimPlot(Seu.Obj.biomart, group.by = "cell_type", label = T)
ggsave(filename = paste(outputFolder,"Dimplot_biomart_10_08_22_celltype.png"), device = "png", width = 10, height = 8)

#para
Seu.Obj.paran <- readRDS("/media/Storage/R/Inparanoid/SeuratObject.combined.integrated_10_08_22_annotated.rds")
DimPlot(Seu.Obj.paran, group.by = "seurat_clusters", label = T)
ggsave(filename = paste(outputFolder,"Dimplot_para_10_08_22_seuratclusters.png"), device = "png", width = 10, height = 8)
DimPlot(Seu.Obj.paran, group.by = "cell_type", label = T)
ggsave(filename = paste(outputFolder,"Dimplot_para_10_08_22_celltype.png"), device = "png", width = 10, height = 8)

#OMA
Seu.Obj.OMA <- readRDS("/media/Storage/R/OMA/SeuratObject.combined.integrated_10_08_22_annotated.rds")
DimPlot(Seu.Obj.OMA, group.by = "seurat_clusters", label = T)
ggsave(filename = paste(outputFolder,"Dimplot_OMA_10_08_22_seuratclusters.png"), device = "png", width = 10, height = 8)
DimPlot(Seu.Obj.OMA, group.by = "cell_type", label = T)
ggsave(filename = paste(outputFolder,"Dimplot_OMA_10_08_22_celltype.png"), device = "png", width = 10, height = 8)
...
##5.7 FIG.4A&B Log2FC Heatmap  (Figure 4A & B)
```{r}
...
##6.2 FIG.2B&C / Bar plot with mouse gtf file for % of mouse genes found with orthologues (Figure 2B & 2C)
```{r}
...
tmp <- GTF.df.m[!duplicated(GTF.df.m$gene_name),]
length(unique(tmp$gene_name))
unique(tmp$gene_type)

GTF.mice.gene.names <- tmp[!grepl("pseudogene",tmp$gene_type),] # 32043

# merge with my data frame orthointegrate to see % of lncRNA and proteincoding run 1.6 to get DF
merge.orthoIntegrate <- GTF.mice.gene.names[GTF.mice.gene.names$gene_name %in% OrthologueList$MouseGene,] #lncRNA 68, protein_coding 17240

#merge with OMA result
OMA_ortholog <- read.delim2(file = "/home/mariano/OMA/OrthologList_OMA_entrez.txt") # does have only 1:1s
mouse_entrez_id <- OMA_ortholog$X66505 
library(org.Mm.eg.db)
gene_names <- select(org.Mm.eg.db, keys = mouse_entrez_id, keytype = "ENTREZID", columns = "SYMBOL")
mouse_gene_names <- gene_names$SYMBOL
mouse_gene_names <- unique(mouse_gene_names)
mouse_gene_names <- mouse_gene_names[!is.na(mouse_gene_names)]
merge.OMA <- GTF.mice.gene.names[GTF.mice.gene.names$gene_name %in% mouse_gene_names,] #lncRNA 4 protein_coding 16713

#justbiomart run code in 12. to get df
merge.biomart <- GTF.mice.gene.names[GTF.mice.gene.names$gene_name %in% OrthologueList$MouseGene,] #lncRNA 5 protein_coding 16108

#InParanoid run code in 12. to get df
OrthologueList <- openxlsx::read.xlsx(xlsxFile = "/media/Helios_scStorage/Mariano/Human_Mice_Comparison/Human_Mice_Integrated/Comparison_other_orthologue_objects/Inparanoid/OrthologueList_Inparanoid.xlsx")
colnames(OrthologueList)  <- c("HGNC.symbol","MouseGene")

merge.InParanoid <- GTF.mice.gene.names[GTF.mice.gene.names$gene_name %in% OrthologueList$MouseGene,] #lncRNA 3 protein_coding 15245

#protein coding plot
prot.DF <- data.frame("OMA" = 16713,
                      "biomart" = 16108,
                      "InParanoid" = 15245,
                      "OrthoIntegrate" =  17240)

prot.DF.m <- melt(prot.DF) # melt
colnames(prot.DF.m) <- c("Tool", "Orthologs")

prot.DF.m$Tool <- factor(prot.DF.m$Tool, levels = unique(prot.DF.m$Tool))
#plot
ggplot(prot.DF.m, aes(fill=Tool, y=Orthologs,x=Tool))+
  geom_col(size = 1, width = 1)+
  scale_fill_manual(values = rep(c("#003f5c" ,"#08519C", "#0DA336", "#ffa600")) 
                      , name = "Tool")+
  labs(title = "", y = "Number of protein coding orthologs")+
  # coord_flip()+
  theme_classic() +
...
ggsave(filename = "/media/Helios_scStorage/Mariano/Human_Mice_Comparison/Human_Mice_Integrated/Comparison_other_orthologue_objects/barplot_protein_orthologs.svg", width = 6,height = 6)


#protein coding plot
lncRNA.DF <- data.frame("OMA" = 4,
                      "biomart" = 5,
                      "InParanoid" = 3,
                      "OrthoIntegrate" =  68)

lncRNA.DF.m <- melt(lncRNA.DF) # melt
colnames(lncRNA.DF.m) <- c("Tool", "Orthologs")

lncRNA.DF.m$Tool <- factor(lncRNA.DF.m$Tool, levels = unique(lncRNA.DF.m$Tool))
#plot
ggplot(lncRNA.DF.m, aes(fill=Tool, y=Orthologs,x=Tool))+
  geom_col(size = 1, width = 1)+
  scale_fill_manual(values = rep(c("#003f5c" ,"#08519C", "#0DA336", "#ffa600")) 
                      , name = "Tool")+
  labs(title = "", y = "Number of lncRNA coding orthologs")+
  # coord_flip()+
  theme_classic() +
  theme(axis.text.x = element_text(face = "bold", color = "black",angle = 45,hjust = 1, size = 15),
          axis.text.y = element_text(color = "black", size = 15),
...
ggsave(filename = "/media/Helios_scStorage/Mariano/Human_Mice_Comparison/Human_Mice_Integrated/Comparison_other_orthologue_objects/barplot_lncRNA_orthologs.svg", width = 6,height = 6)

```

##6.3 Making a SCO for scanpy transfer
```{r}
outputFolder2 <- "/media/Storage/R/Revision_scib/"
library(anndata)
library(Seurat)
library(zellkonverter)
#Read in Object to use ZELLKONVERTER
Seu.Obj <- readRDS("/media/Storage/R/Integrated/SeuratObject.combined.integrated.annotated_10_08_22.rds")
DefaultAssay(object = Seu.Obj) <- "RNA"

#Get uMAP embeddings, PCA embeddings, k-param neares neighborhood
UMAPEmbeddings <- Seu.Obj@reductions$umap
UMAPEmbeddings <- UMAPEmbeddings@cell.embeddings
write.csv(UMAPEmbeddings, file = "/media/Storage/R/Integrated/UMAPEmbedings.csv", row.names = T)
#PCA
PCAEmbeddings <- Seu.Obj@reductions$pca
PCAEmbeddings <- PCAEmbeddings@cell.embeddings
write.csv(PCAEmbeddings, file = "/media/Storage/R/Integrated/PCAEmbedings.csv", row.names = T)
#Convert object
DefaultAssay(Seu.Obj) <- "RNA"
tmp.sce <- Seurat::as.SingleCellExperiment(Seu.Obj)
zellkonverter::writeH5AD(tmp.sce, file = paste0(outputFolder2,"Conversion/SeuratObject.combined.integrated.annotated_10_08_22.h5ad"), X_name = "logcounts")

#Repeat for other Objects
Seu.Obj.biomart <- readRDS("/media/Storage/R/just_biomart/SeuratObject.combined.integrated_10_08_22_annotated.rds")
DefaultAssay(Seu.Obj.biomart) <- "RNA"
Seu.Obj.paran <- readRDS("/media/Storage/R/Inparanoid/SeuratObject.combined.integrated_10_08_22_annotated.rds")
DefaultAssay(Seu.Obj.paran) <- "RNA"
Seu.Obj.OMA <- readRDS("/media/Storage/R/OMA/SeuratObject.combined.integrated_10_08_22_annotated.rds")
DefaultAssay(Seu.Obj.OMA) <- "RNA"

#Get uMAP embeddings, PCA embeddings, k-param neares neighborhood
UMAPEmbeddings <- Seu.Obj.biomart@reductions$umap
UMAPEmbeddings <- UMAPEmbeddings@cell.embeddings
write.csv(UMAPEmbeddings, file = "/media/Storage/R/just_biomart/UMAPEmbedings.csv", row.names = T)
#PCA
PCAEmbeddings <- Seu.Obj.biomart@reductions$pca
PCAEmbeddings <- PCAEmbeddings@cell.embeddings
write.csv(PCAEmbeddings, file = "/media/Storage/R/just_biomart/PCAEmbedings.csv", row.names = T)
tmp.sce <- Seurat::as.SingleCellExperiment(Seu.Obj.biomart)
zellkonverter::writeH5AD(tmp.sce, file = paste0(outputFolder2,"Conversion/SeuratObject.biomart_10_08_22.h5ad"), X_name = "logcounts")

#Get uMAP embeddings, PCA embeddings, k-param neares neighborhood
UMAPEmbeddings <- Seu.Obj.paran@reductions$umap
UMAPEmbeddings <- UMAPEmbeddings@cell.embeddings
write.csv(UMAPEmbeddings, file = "/media/Storage/R/Inparanoid/UMAPEmbedings.csv", row.names = T)
#PCA
PCAEmbeddings <- Seu.Obj.paran@reductions$pca
PCAEmbeddings <- PCAEmbeddings@cell.embeddings
write.csv(PCAEmbeddings, file = "/media/Storage/R/Inparanoid/PCAEmbedings.csv", row.names = T)
tmp.sce <- Seurat::as.SingleCellExperiment(Seu.Obj.paran)
zellkonverter::writeH5AD(tmp.sce, file = paste0(outputFolder2,"Conversion/SeuratObject.inparanoid_10_08_22.h5ad"), X_name = "logcounts")

#Get uMAP embeddings, PCA embeddings, k-param neares neighborhood
UMAPEmbeddings <- Seu.Obj.OMA@reductions$umap
UMAPEmbeddings <- UMAPEmbeddings@cell.embeddings
write.csv(UMAPEmbeddings, file = "/media/Storage/R/OMA/UMAPEmbedings.csv", row.names = T)
#PCA
PCAEmbeddings <- Seu.Obj.OMA@reductions$pca
PCAEmbeddings <- PCAEmbeddings@cell.embeddings
write.csv(PCAEmbeddings, file = "/media/Storage/R/OMA/PCAEmbedings.csv", row.names = T)
tmp.sce <- Seurat::as.SingleCellExperiment(Seu.Obj.OMA)
zellkonverter::writeH5AD(tmp.sce, file = paste0(outputFolder2,"Conversion/SeuratObject.oma_10_08_22.h5ad"), X_name = "logcounts")


rm(tmp.sce)
```

##6.4 need unintegrated objects for metric calculation of scib package in python
```{r}
outputFolder2 <- "/media/Storage/R/Revision_scib/"
library(anndata)
library(Seurat)
...
#need unintegrated Anndata object, create SCE objects out of my SeuratObjectList before integration
SeuratObjectList <- readRDS(file = "/media/Helios_scStorage/Mariano/Human_Mice_Comparison/Human_Mice_Integrated/Seurat_and_R_objects/SeuratObjectList_before_Integration.rds")

for(seuObj in SeuratObjectList){
  orig.ident <- unique(seuObj$orig.ident) #gives error for control groups
  tmp.sce <- Seurat::as.SingleCellExperiment(seuObj)
  zellkonverter::writeH5AD(tmp.sce, file = paste0(outputFolder2,"Conversion/unintegrated/Seu_Obj_",orig.ident,".h5ad"), X_name = "logcounts")
}
```

##6.5 FIG.2D / Table out of metrics calculated by python script (Figure 2D)
```{r}
library(openxlsx)
library(tidyverse)
library(reactable)
library(htmltools)
metric_com <- read.csv("/Users/mariano/Desktop/Work_PhD/Project/R_analysis/Revision_scib/metric_comb_cell_type_last.csv")
metric_com <- metric_com %>% rename(Tool = X)
#include values of calculation in python
#metric_com[["ARI"]] <- c(0.5189914261303687, 0.6193180164344755, 05628124884257347, 0.6436776622868948)
#metric_com[["NMI"]] <- c(0.7786952496474926, 0.8443411899971773, 0.7947436629297074, 0.8434689168247185 )
#exclude LISI as proposed by BENGAL
metric_com[["LISI"]] <- NULL
metric_com[["Species.type.LISI"]] <- NULL
metric_com[["ARI"]] <- NULL

#metric_com <- metric_com[,c(1,2,3,4,5,6,7,8,12,13,9,11,10)]
reorder_vec <- c("OMA","biomart","InParanoid","OrthoIntegrate")
metric_com <- metric_com[order(match(metric_com$Tool, reorder_vec)), ]
metric_com$Tool <- c("OMA","Biomart","InParanoid","OrthoInt.")

#recalculate combined score
#metric_com$Species.mixing.score <- rowSums(metric_com[,2:5])/4
metric_com$Bio.Conservation.Score <- rowSums(metric_com[,6:9])/4
metric_com$Total.Score <- (metric_com$Species.mixing.score * 0.5) + (metric_com$Bio.Conservation.Score *0.5)

#write.csv(metric_com, file = "/Users/mariano/Desktop/Work_PhD/Project/R_analysis/Revision_scib/metric_com_WITH_NMI_ARI_without_LISI.csv")

batch_cols <- c("graph_connectivity", "pcr_comparison", "silhouette_batch", "kBET")
bio_cols <- c("Cell.cycle.conservation", "isolated_labels_f1", "Silhouette.Coefficient","NMI")
aggregate_cols <- c("Species.mixing.score", "Bio.Conservation.Score", "Total.Score")

batch_column <- function(maxWidth = 3000, ...) {
  colDef(maxWidth = maxWidth, align = "center", class = "cell number", ...)
}

bio_column <- function(maxWidth = 3000, ...) {
  colDef(maxWidth = maxWidth, align = "center", class = "cell number", ...)
}

aggregate_column <- function(maxWidth = 3000, ...) {
  colDef(maxWidth = maxWidth, align = "center", class = "cell number", ...)
}
...
off_rating_color <- make_color_pal(c("#287D8EFF", "#f8fcf8", "#3CBB75FF"), bias = 1)
score_rating_color <- make_color_pal(c("#FDE725FF", "#f8fcf8", "#bda78f"), bias = 23)
score_rating_final <- make_color_pal(c("#FDE725FF", "#f5e241","#fdf5b2"), bias = 23)
...
reactable(
  ...
  columnGroups = list(
    colGroup(name = "Batch Correction", columns = batch_cols),
    colGroup(name = "Bio Conservation", columns = bio_cols),
    colGroup(name = "Aggregate Score", columns = aggregate_cols)
  ),
  ...
  columns = list(
    ...
    graph_connectivity = batch_column(
      name = "GC",
      width = 100,  # Adjust the width as needed
      cell = function(value) {
        scaled <- (value - min(metric_com$graph_connectivity)) / (max(metric_com$graph_connectivity) - min(metric_com$graph_connectivity))
        color <- "#287D8EFF"
        value <- format(round(value, 3), nsmall = 1)
        div(class = "spi-rating", style = list(background = color), value)
      }
    ),
    pcr_comparison = batch_column(
      name = "PCR", 
      defaultSortOrder = "asc",
      width = 100,  # Adjust the width as needed
      cell = function(value) {
        scaled <- 1 - (value - min(metric_com$pcr_comparison)) / (max(metric_com$pcr_comparison) - min(metric_com$pcr_comparison))
        color <- off_rating_color(scaled)
        value <- format(round(value, 3), nsmall = 1)
        div(class = "spi-rating", style = list(background = color), value)
      }
    ),
    silhouette_batch = batch_column(
      name = "bASW", 
      defaultSortOrder = "asc",
      width = 100,  # Adjust the width as needed
      cell = function(value) {
        scaled <- 1 - (value - min(metric_com$silhouette_batch)) / (max(metric_com$silhouette_batch) - min(metric_com$silhouette_batch))
        color <- off_rating_color(scaled)
        value <- format(round(value, 3), nsmall = 1)
        div(class = "spi-rating", style = list(background = color), value)
      }
      ...
      defaultSortOrder = "asc",
      cell = function(value) {
        scaled <- 1 - (value - min(metric_com$kBET)) / (max(metric_com$kBET) - min(metric_com$kBET))
        color <- off_rating_color(scaled)
        value <- format(round(value, 3), nsmall = 1)
        div(class = "spi-rating", style = list(background = color), value)
      }
    ...
    #   defaultSortOrder = "asc",
    #   cell = function(value) {
    #     scaled <- 1 - (value - min(metric_com$ARI)) / (max(metric_com$ARI) - min(metric_com$ARI))
    #     color <- off_rating_color(scaled)
    #     value <- format(round(value, 3), nsmall = 1)
    #     div(class = "spi-rating", style = list(background = color), value)
    #   }
    # ),
      Cell.cycle.conservation = bio_column(
      name = "CCC", 
      width = 100,  # Adjust the width as needed
      defaultSortOrder = "asc",
      cell = function(value) {
        scaled <- 1 - (value - min(metric_com$Cell.cycle.conservation)) / (max(metric_com$Cell.cycle.conservation) - min(metric_com$Cell.cycle.conservation))
        color <- off_rating_color(scaled)
        value <- format(round(value, 3), nsmall = 1)
        div(class = "spi-rating", style = list(background = color), value)
      }
      ...
      defaultSortOrder = "asc",
      cell = function(value) {
        scaled <- 1 - (value - min(metric_com$NMI)) / (max(metric_com$NMI) - min(metric_com$NMI))
        color <- off_rating_color(scaled)
        value <- format(round(value, 3), nsmall = 1)
        div(class = "spi-rating", style = list(background = color), value)
      }
    ),
      isolated_labels_f1 = bio_column(
      name = "ILF1", 
      width = 100,  # Adjust the width as needed
      defaultSortOrder = "asc",
      cell = function(value) {
        scaled <- 1 - (value - min(metric_com$isolated_labels_f1)) / (max(metric_com$isolated_labels_f1) - min(metric_com$isolated_labels_f1))
        color <- off_rating_color(scaled)
        value <- format(round(value, 3), nsmall = 1)
        div(class = "spi-rating", style = list(background = color), value)
      }
    ),
      Silhouette.Coefficient = bio_column(
      name = "SC", 
      width = 100,  # Adjust the width as needed
      defaultSortOrder = "asc",
      cell = function(value) {
        scaled <- 1 - (value - min(metric_com$Silhouette.Coefficient)) / (max(metric_com$Silhouette.Coefficient) - min(metric_com$Silhouette.Coefficient))
        color <- off_rating_color(scaled)
        value <- format(round(value, 3), nsmall = 1)
        div(class = "spi-rating", style = list(background = color), value)
      }
    ),
      Bio.Conservation.Score = aggregate_column(
      name = "BSC", 
      width = 100,  # Adjust the width as needed
      defaultSortOrder = "asc",
      cell = function(value) {
        scaled <- 1 - (value - min(metric_com$Bio.Conservation.Score)) / (max(metric_com$Bio.Conservation.Score) - min(metric_com$Bio.Conservation.Score))
        color <- score_rating_color(scaled)
        value <- format(round(value, 3), nsmall = 1)
        div(class = "spi-rating", style = list(background = color), value)
      }
    ),
      Total.Score = aggregate_column(
      name = "Total", 
      width = 100,  # Adjust the width as needed
      defaultSortOrder = "asc",
      cell = function(value) {
        scaled <- 1 - (value - min(metric_com$Total.Score)) / (max(metric_com$Total.Score) - min(metric_com$Total.Score))
        color <- score_rating_final(scaled)
        value <- format(round(value, 3), nsmall = 1)
        div(class = "spi-rating", style = list(background = color), value)
      }
    ),
      Species.mixing.score = aggregate_column(
      name = "SMS", 
      width = 100,  # Adjust the width as needed
      defaultSortOrder = "asc",
      cell = function(value) {
        scaled <- 1 - (value - min(metric_com$Species.mixing.score)) / (max(metric_com$Species.mixing.score) - min(metric_com$Species.mixing.score))
        color <- score_rating_color(scaled)
        value <- format(round(value, 3), nsmall = 1)
        div(class = "spi-rating", style = list(background = color), value)
      }
  ...
  compact = TRUE,
  rowStyle = function() {
    list(marginBottom = "16px")  # Adjust the margin as needed
  },
  theme = reactableTheme(
            borderColor = "#dfe2e5",
            stripedColor = "#f6f8fa",
...
```
#7. SUPP.FIG.4A&B / Build UpSet plot with DEG list (Supplement Figure 4A & B)
